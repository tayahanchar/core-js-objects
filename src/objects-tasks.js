...
function makeWord(lettersObject) {
  const word = [];
  Object.entries(lettersObject).forEach((letterArray) => {
    const [letter, positions] = letterArray;
...
function sellTickets(queue) {
  ...
  const payment = queue.pop();

  return queue.reduce((acc, item) => acc + item, 0) >= payment;
}
